...
exports.getEmployeeGroupInformation = async (req, res, next) => {
    ...
    return (async () => {
        ...
        let { emp_group_code, off_code, page_index, page_limit, action } = req.body[0];
        ...
        if (emp_group_code == undefined || off_code == undefined || lic_code == undefined || action == undefined) {
            let response = [{
                status: 'error',
                invalid_code: '-1',
            ...
            let script = ``;
            if (emp_group_code.toString().toUpperCase() != 'ALL') {
                script = `select emp_group_code, emp_group_desc, emp_group_flag,
                ist_dt, mdf_dt, rm_dt from tbl_employee_group 
                where emp_group_flag = '1' and emp_group_code = '${emp_group_code}'`;
            }
            else {
                script = `select emp_group_code, emp_group_desc, emp_group_flag,
                ist_dt, mdf_dt, rm_dt from tbl_employee_group where emp_group_flag = '1'`;
            }

            if (off_code.toString().toUpperCase() != 'ALL') {
                script += ` and tbl_employee_group.off_code = '${off_code}'`
            }

            script += `  order by tbl_employee_group.ist_dt desc`
            ...
            if (!tbl_temporary.code) {
                ...
                if (tbl_temporary.data.length > 0) {
                    ...
                    let script = ``
                    if (emp_group_code.toString().toUpperCase() != 'ALL') {
                        script = `select ceil((ceil(count(emp_group_code)) / ${page_limit})) as page_total, (count(emp_group_code)) as rows_total 
                    from tbl_employee_group 
                    where emp_group_flag = '1' and emp_group_code = '${emp_group_code}'`;
                    }
                    else {
                        script = `select ceil((ceil(count(emp_group_code)) / ${page_limit})) as page_total, (count(emp_group_code)) as rows_total 
                    from tbl_employee_group where emp_group_flag = '1'`;
                    }
...
exports.removeEmployeeGroup = async (req, res, next) => {
    ...
    return (async () => {
        let lic_code = req.header('lic_code');
        let { emp_group_code, action } = req.body[0];
        //เช็คเฉพาะส่วนที่สำคัญ
        if (emp_group_code == undefined || lic_code == undefined || action == undefined) {
            let response = [{
                status: 'error',
                invalid_code: '-1',
            ...
            let script = ``;
            script = `update tbl_employee_group set emp_group_flag = '0', rm_dt = '${moment().format('YYYY-MM-DD HH:mm:ss')}' where emp_group_code = '${emp_group_code}';`
...
exports.setEmployeeGroupInformation = async (req, res, next) => {
    ...
    return (async () => {
        ...
        //เช็คเฉพาะส่วนที่สำคัญ
        if (emp_group_code == undefined || off_code == undefined || emp_group_desc == undefined || action == undefined) {
            let response = [{
                status: 'error',
                invalid_code: '-1',
            ...
            script = `update tbl_employee_group set
            emp_group_desc = '${emp_group_desc}',
            off_code = '${off_code}',
            mdf_dt = '${moment().format('YYYY-MM-DD HH:mm:ss')}' 
            where emp_group_code = '${emp_group_code}';`
...
exports.addEmployeeGroupInformation = async (req, res, next) => {
    ...
    return (async () => {
        ...
        //เช็คเฉพาะส่วนที่สำคัญ
        if (emp_group_desc == undefined || off_code == undefined || action == undefined) {
            let response = [{
                status: 'error',
                invalid_code: '-1',
                message: 'ไม่สามารถบันทึกข้อมูล, เนื่องจากข้อมูลพารามิเตอร์ไม่ถูกต้อง',
            ...
            script = `select emp_group_code from tbl_employee_group where emp_group_desc = '${emp_group_desc}' and off_code = '${off_code}' and emp_group_flag = '1';`
            ...
            let emp_group_code = 'emgp-' + moment().format('x');
            script = `insert into tbl_employee_group 
            (emp_group_code, emp_group_desc, emp_group_flag, ist_dt, off_code) values 
            ('${emp_group_code}', '${emp_group_desc}', '1', '${moment().format('YYYY-MM-DD HH:mm:ss')}', '${off_code}');`

...
exports.getVehicleTypeInformation = async (req, res, next) => {
    ...
    return (async () => {
        ...
        if (veh_type_code == undefined || lic_code == undefined || action == undefined) {
            ...
            script += ` order by veh_type_desc asc;`
...
exports.setVehicleTypeInformation = async (req, res, next) => {
    ...
    return (async () => {
        ...
        //เช็คเฉพาะส่วนที่สำคัญ
        if (veh_type_code == undefined || veh_type_desc == undefined || action == undefined) {
            let response = [{
                status: 'error',
                invalid_code: '-1',
...
exports.addVehicleTypeInformation = async (req, res, next) => {
    ...
    return (async () => {
        ...
        //เช็คเฉพาะส่วนที่สำคัญ
        if (veh_type_desc == undefined || action == undefined || unloading_minute == undefined || loading_minute == undefined) {
            let response = [{
                status: 'error',
                invalid_code: '-1',
                message: 'ไม่สามารถบันทึกข้อมูล, เนื่องจากข้อมูลพารามิเตอร์ไม่ถูกต้อง',
            ...
            let script = ``;
            script = `select veh_type_code from tbl_vehicle_type 
            where veh_type_desc = '${veh_type_desc}' and veh_type_flag = '1';`
            ...
            if (!tbl_temporary0.code) {
                if (tbl_temporary0.data.length > 0) {
                    ...
                    res.status(200).send(response);
                    await xglobal.action_logs(lic_code, action[0].id, 'เพิ่มข้อมูลประเภทรถ', JSON.stringify(req.body[0]), 'ไม่สามารถบันทึกข้อมูล, เนื่องจากข้อมูลซ้ำ', action[0].value);
                    return;
                }
            }
            ...
            if (!tbl_temporary.code) {
                ...
                res.status(200).send(response);
                await xglobal.action_logs(lic_code, action[0].id, 'เพิ่มข้อมูลประเภทรถ', JSON.stringify(req.body[0]), 'ไม่สามารถบันทึกข้อมูล, กรุณาติดต่อเจ้าหน้าที่ผู้ดูแลระบบ', action[0].value);
                return;
            }
        }

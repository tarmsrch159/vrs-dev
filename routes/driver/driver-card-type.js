...
//Success
exports.getDriverCardTypeInformation = async (req, res, next) => {
    ...
    return (async () => {
        ...
        let { dver_card_type_code, action } = req.body[0];
        ...
        if (dver_card_type_code == undefined || lic_code == undefined || action == undefined) {
            ...
            if (dver_card_type_code.toString().toUpperCase() != 'ALL') {
                script = `select dver_card_type_code, dver_card_type_desc, dver_card_type_flag,
                ist_dt, mdf_dt, rm_dt from tbl_driver_card_type 
...
//Success
exports.removeDriverCardType = async (req, res, next) => {
    ...
    return (async () => {
        ...
        if (dver_card_type_code == undefined || lic_code == undefined || action == undefined) {
            ...
            let script = ``;
            script = `update tbl_driver_card_type set dver_card_type_flag = '0', rm_dt = '${moment().format('YYYY-MM-DD HH:mm:ss')}' where dver_card_type_code = '${dver_card_type_code}';`
...
//Success
exports.setDriverCardTypeInformation = async (req, res, next) => {

    return (async () => {
        //debugger
        let lic_code = req.header('lic_code');
        let { dver_card_type_code } = req.query;
        let { dver_card_type_desc, action } = req.body[0];

        //เช็คเฉพาะส่วนที่สำคัญ
        if (dver_card_type_code == undefined || dver_card_type_desc == undefined || action == undefined) {
            let response = [{
                status: 'error',
                invalid_code: '-1',
            ...
            script = `update tbl_driver_card_type set
            dver_card_type_desc = '${dver_card_type_desc}',
            mdf_dt = '${moment().format('YYYY-MM-DD HH:mm:ss')}' 
            where dver_card_type_code = '${dver_card_type_code}';`
...
//Success
exports.addDriverCardTypeInformation = async (req, res, next) => {

    return (async () => {
        //debugger
        let lic_code = req.header('lic_code');
        let {
            dver_card_type_desc, action
        } = req.body[0];

        //เช็คเฉพาะส่วนที่สำคัญ
        if (dver_card_type_desc == undefined || lic_code == undefined || action == undefined) {
            let response = [{
                status: 'error',
                invalid_code: '-1',
                message: 'ไม่สามารถบันทึกข้อมูล, เนื่องจากข้อมูลพารามิเตอร์ไม่ถูกต้อง',
            ...
            script = `select dver_card_type_code from tbl_driver_card_type where dver_card_type_desc = '${dver_card_type_desc}' and dver_card_type_flag = '1';`
            ...
            let dver_card_type_code = 'clic-' + moment().format('x');
            script = `insert into tbl_driver_card_type 
            (dver_card_type_code, dver_card_type_desc, dver_card_type_flag, ist_dt) values 
            ('${dver_card_type_code}', '${dver_card_type_desc}', '1', '${moment().format('YYYY-MM-DD HH:mm:ss')}');`

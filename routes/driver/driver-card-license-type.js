...
exports.getDriverLicenseTypeInformation = async (req, res, next) => {
    ...
    return (async () => {
        ...
        let { dver_card_license_type_code, action, page_index, page_limit } = req.body[0];
        ...
        if (dver_card_license_type_code == undefined || lic_code == undefined || action == undefined) {
            ...
            script += ` order by tbl_driver_card_license_type.ist_dt desc`
            ...
            if (!tbl_temporary.code) {
                ...
                if (tbl_temporary.data.length > 0) {
                    ...
                    if (dver_card_license_type_code.toString().toUpperCase() != 'ALL') {
                        ...
                        ceil(count(dver_card_license_type_code)::numeric / ${page_limit}) as page_total from tbl_driver_card_license_type 
                        where dver_card_license_type_flag = '1' and dver_card_license_type_code = '${dver_card_license_type_code}' `;
                    }
                    else {
                        script = `select count(*) as rows_total,
                        ceil(count(dver_card_license_type_code)::numeric / ${page_limit}) as page_total from tbl_driver_card_license_type where dver_card_license_type_flag = '1' `;
                    }

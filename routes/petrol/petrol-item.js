...
exports.getPetrolItemInformation = async (req, res, next) => {
    ...
    return (async () => {
        ...
        let { itm_code, ptrl_code, action } = req.body[0];
        ...
        if (itm_code == undefined || ptrl_code == undefined || lic_code == undefined || action == undefined) {
            ...
            if (itm_code.toString().toUpperCase() != 'ALL') {
                ...
                tbl_petrol.off_code,
                off_desc,
                tbl_petrol_item.ist_dt,
                tbl_petrol_item.mdf_dt,
                tbl_petrol_item.rm_dt 
                from tbl_petrol
                ...
                left join tbl_item on tbl_petrol_item.itm_code = tbl_item.itm_code 
                ...
                where tbl_petrol_item.itm_code = '${itm_code}' and tbl_petrol_item.ptrl_item_flag = '1' and ptrl_item_code is not null `;
            }
            else {
                script = `select
                ptrl_item_code,
                ...
                left join tbl_item_type on tbl_item.itm_type_code = tbl_item_type.itm_type_code 
                left join tbl_petrol_group on tbl_petrol.ptrl_group_code = tbl_petrol_group.ptrl_group_code 
                where tbl_petrol_item.ptrl_item_flag = '1' and ptrl_item_code is not null `;
            }
...
exports.removePetrolItem = async (req, res, next) => {
    ...
    return (async () => {
        ...
        if (ptrl_item_code == undefined || lic_code == undefined || action == undefined) {
            ...
            let script = ``;
            script = `update tbl_petrol_item set ptrl_item_flag = '0', rm_dt = '${moment().format('YYYY-MM-DD HH:mm:ss')}' where ptrl_item_code = '${ptrl_item_code}';`
...
exports.addPetrolItemInformation = async (req, res, next) => {
    ...
    return (async () => {
        ...
        if (ptrl_code == undefined || itm_code == undefined || action == undefined || lic_code == undefined) {
            ...
            let script = ``;
            script = `select ptrl_item_code from tbl_petrol_item where ptrl_item_flag = '1' and ptrl_code = '${ptrl_code}' and itm_code = '${itm_code}';`
            ...
            if (!tbl_temporary0.code) {
                if (tbl_temporary0.data.length > 0) {
                    ...
                    res.status(200).send(response);
                    await xglobal.action_logs(lic_code, action[0].id, 'เพิ่มข้อมูลสินค้าปั้ม', JSON.stringify(req.body[0]), 'ไม่สามารถบันทึกข้อมูล, เนื่องจากข้อมูลซ้ำ', action[0].value);
                    return;
                }
            }

            let ptrl_item_code = 'pitm-' + moment().format('x');
            script = `insert into tbl_petrol_item 
            (ptrl_item_code, ptrl_code, itm_code, ptrl_item_flag, ist_dt) values 
            ('${ptrl_item_code}', '${ptrl_code}', '${itm_code}', '1', '${moment().format('YYYY-MM-DD HH:mm:ss')}');`
